// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["maotai","douban", "guangsu", "subo"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["maotai", "douban", "guangsu", "subo"];
...
function addAdultAPI() {
    ...
    if (!HIDE_BUILTIN_ADULT_APIS && (localStorage.getItem('yellowFilterEnabled') === 'false')) {
        ...
        adultTitle.innerHTML = `色彩资源采集站 <span class="adult-warning">
...
function checkAdultAPIsSelected() {
    ...
    if (hasAdultSelected) {
        ...
        if (filterDescription) {
            filterDescription.innerHTML = '<strong class="text-pink-300">选中色彩资源站时无法启用此过滤</strong>';
        }
        ...
        // 恢复原来的描述文字
        if (filterDescription) {
            filterDescription.innerHTML = '过滤"伦理片"等色彩内容';
        }
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {},
            `ShabiTV - 免费在线视频搜索与观看平台`,
            `/`
        );
        // 更新页面标题
        document.title = `ShabiTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - ShabiTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }

        // 处理搜索结果过滤：如果启用了色彩内容过滤，则过滤掉分类含有敏感内容的项目
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'ShabiTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'ShabiTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'ShabiTV-Settings_' + times + '.json');
}
